%% TK2nii.m
function voxcoord = TK2nii_batch(tklist,refNII,savename,donecheckname)
%% Instructions
...
for k = 1:numel(varnames);
    ...
    dummyimg = img.(sprintf(varnames{k}));
    buildingimg = dummyimg + buildingimg;
    
end

dummynii = make_nii(buildingimg,[],origin);
%dummynii.img(dummynii.img == 0) = nan;
save_nii(dummynii,savename);
